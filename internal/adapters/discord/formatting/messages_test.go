...
func TestConstants(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "MsgGuildNameRequired",
			constant: MsgGuildNameRequired,
			expected: "Guild name is required.",
		},
		{
			name:     "MsgSaveError",
		...
		{
			name:     "MsgConfigError",
			constant: MsgConfigError,
			expected: "Failed to retrieve configuration.",
		},
		{
			name:     "MsgNoGuildsTracked",
			constant: MsgNoGuildsTracked,
			expected: "No guilds are currently being tracked (all players will be tracked).",
		},
	}
...
func TestMsgGuildAdded(t *testing.T) {
	tests := []struct {
		name     string
		guild    string
		expected string
	}{
		{"standard guild", "Red Rose", "Added guild 'Red Rose' to tracking list."},
		{"guild with special chars", "The O'Reilly's", "Added guild 'The O'Reilly's' to tracking list."},
		{"short name", "PK", "Added guild 'PK' to tracking list."},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if result := MsgGuildAdded(tt.guild); result != tt.expected {
				t.Errorf("Expected '%s', got '%s'", tt.expected, result)
			}
		})
	}
...
func TestMsgGuildRemoved(t *testing.T) {
	tests := []struct {
		name     string
		guild    string
		expected string
	}{
		{"standard guild", "Red Rose", "Removed guild 'Red Rose' from tracking list."},
		{"guild with unicode", "Śląsk Warriors", "Removed guild 'Śląsk Warriors' from tracking list."},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if result := MsgGuildRemoved(tt.guild); result != tt.expected {
				t.Errorf("Expected '%s', got '%s'", tt.expected, result)
			}
		})
	}
...
func TestMsgGuildsList(t *testing.T) {
	tests := []struct {
		name     string
		guilds   []string
		expected string
		...
		{
			name:     "single guild",
			guilds:   []string{"Red Rose"},
			expected: "Tracking specific guilds:\n- Red Rose\n",
		},
		{
			name:     "multiple guilds",
			guilds:   []string{"Red Rose", "Eternal Flames", "Dark Side"},
			expected: "Tracking specific guilds:\n- Red Rose\n- Eternal Flames\n- Dark Side\n",
		},
		{
			name:     "empty list",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if result := MsgGuildsList(tt.guilds); result != tt.expected {
				t.Errorf("Expected '%s', got '%s'", tt.expected, result)
			}
		})
	}
